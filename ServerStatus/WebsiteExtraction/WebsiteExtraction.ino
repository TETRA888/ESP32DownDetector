...
void SendRequestToServer(){
  ...
  if (!client.connect(server, 443))
    ...
    display.println(F("Connected to server!"));
  ...
  client.println();
    ...
    while (client.connected()) {
      ...
      if (line == "\r") {
        display.println(F("headers received"));
        break;
      }
    }
      ...
      htmlContent += (char)client.read();
      }

      // Parse and extract text from span tags
      //Serial.println("\n--- Content in <span> tags ---");
      int startPos = 0;

      while (true) {
        // Find opening span tag
        int spanStart = htmlContent.indexOf("<span", startPos);
        if (spanStart == -1) break;

        // Find end of opening tag
        int contentStart = htmlContent.indexOf(">", spanStart);
        if (contentStart == -1) break;
        contentStart++; // Move past the '>'

        // Find closing span tag
        int contentEnd = htmlContent.indexOf("</span>", contentStart);
        if (contentEnd == -1) break;

        // Extract and print the content
        String spanContent = htmlContent.substring(contentStart, contentEnd);
        display.println(F(spanContent));
